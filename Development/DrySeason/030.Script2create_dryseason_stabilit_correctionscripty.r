...
#apply correction to current sd

	for(i in 1:30) { cat(i,'\n')
		tdata=NULL
		tdata=as.data.frame(month.max.clust[,i]-month.max.clust[,31])
		month.max.clust=cbind(month.max.clust, tdata)
		
		
		
		
		colnames(month.max.clust)[i+32]=paste(colnames(month.max.clust) [i],"_centered", sep='')
	}

month.max.clust$sd_centered = apply(month.max.clust[,33:62],1,sd)
	
current_fut_month_max_clust=month.max.clust[,c("month.max.clust.mean","sd_centered")]

data.dir="/home/jc246980/DrySeason/Futuredat/Data/"
vois_data=c("fut_month_max_clust")
