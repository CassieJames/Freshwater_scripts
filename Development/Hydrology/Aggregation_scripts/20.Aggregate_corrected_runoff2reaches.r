################################################################################
# aggregate corrected runoff generated at 5km resolution onto Janets reaches
...
Qrun_curmean=Qrun
	...
	for (ii in 1:ncol(Qrun_curmean)){
		...
		tdata=pos 																		  # make a copy of pos with UID appended
		tdata$runoff_rate=Qrun_corrected_1960[,ii] 										  # Append gridded runoff data
		FINAL<- merge(Area_agg, tdata, by='UID')                                          # Merge Area_agg with 5km pos file                            
		FINAL$Runoff = (FINAL$AREA/1000000) * FINAL$runoff_rate                           # Multiply area(in km) by runoff (in mm/km)
		Reach_runoff_final = aggregate(FINAL$Runoff, by = list(FINAL$SegmentNo), sum) 	  # Aggregate runoff to reaches
		Runoff=cbind(Runoff, Reach_runoff_final$x)
...
###########Go through futures
load("/home/jc246980/Hydrology.trials/Aggregate_reach/Output_futures/Qrun_aggregated2reach_1976to2005/Current_dynamic.Rdata")  # Load runoff
Runoff_agg=Runoff
load("/home/jc246980/Hydrology.trials/Aggregate_reach/Output_futures/Qrun_aggregated2reach_1976to2005/Current_static.Rdata")  # Load runoff
Runoff_stat=Runoff
	...
	for(es in ESs) { 
		...
		for(gcm in GCMs) {
			...
			Runoff=NULL
			Runoff=cbind(Runoff,Runoff_stat) # append 
			Runoff=cbind(Runoff,Runoff_agg) 
		...
		tt = expand.grid(mm,YEARs=YEARs);tt = paste(tt[,2],tt[,1],sep='_'); colnames(Runoff) = tt #add the column names
		save(Runoff, file=paste(out.dir,es,"_",gcm,".Rdata",sep='')) #save the runoff	
