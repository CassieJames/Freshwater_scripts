...
for(i in 2:ncol(species.data.final)) {
		...
		if(i==2){ crayfish.data=temp
	}else{crayfish.data=merge(crayfish.data, temp, by="SegmentNo", all.x=TRUE)
	}
	}
	
colnames(crayfish.data) = colnames(species.data.final)


out.dir="/home/jc246980/Zonation/"
save(crayfish.data,file=paste(out.dir,'Crayfish_reach_aggregated.Rdata',sep=''))     # save