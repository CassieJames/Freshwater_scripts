...
library(SDMTools);library(plotrix); library(maptools); library(foreign)
...
##### Create plots
	...
	png(paste(es,'_',tax,'_',yr,'.July2016.png',sep=''),width=dim(base.asc)[1]*2+300, height=dim(base.asc)[2]*2+150, units='px', pointsize=20, bg='WHITE')	
...
rivers = readShapeLines('/home/jc246980/Janet_Stein_data/Major_rivers/Major_rivers.shp') #read in your shapefile
image.dir = '/home/jc246980/SDM/Paper_images/'; setwd(image.dir)
...
assign.list(min.lon,max.lon,min.lat,max.lat) %=% c( 140,146.65, -21.00, -10.55)
xlim=c(min.lon,max.lon);
ylim=c(min.lat,max.lat)


	  png(paste(es,'_Figure 1_zoom_July2016.png',sep=''),width=dim(base.asc)[1]*2+300, height=dim(base.asc)[2]*2+150, units='px', pointsize=20, bg='WHITE')	
	  par(mar=c(2,2,2,2),cex=1,oma=c(3,0,1,0), bg = 'white')
		...
		mat = matrix(c( 1,1,1,1,1,2,2,2,2,
						...
						1,1,1,1,1,3,3,3,3,
						1,1,1,1,1,3,3,3,3,
						1,1,1,1,1,1,4,4,4),nr=9,nc=9,byrow=TRUE) #create a layout matrix for images
		layout(mat) #call layout as defined above

		

		assign.list(min.lon,max.lon,min.lat,max.lat) %=% c( 140.00,146.65, -21.00, -10.55)
		xlim=c(min.lon,max.lon);
		ylim=c(min.lat,max.lat)			
		...
		color.legend(139.3,-12.0,141.3,-11.7,zlim,cols,cex=10)
		text(139.3,-10.8,"(a)",cex=14)
		
 
		image(base.asc,ann=F,axes=F,col='wheat1')
		image(clip.image(139.5,146.65, -20.05, -10.55),ann=FALSE,axes=FALSE, col="grey80",add=TRUE)
		plot(Drainageshape , lwd=10, ann=FALSE,axes=FALSE, add=TRUE)
		...
		text(115,-10.635,"(b)",cex=14)
		
		cols=colorRampPalette(c("#FFE4B5","#668B8B","#8B3626"))(20)
		image(base.asc,ann=F,axes=F,col='wheat1')
		terrain = read.dbf('/home/jc246980/Janet_Stein_data/Terrain.dbf')
		pos=merge(pos,terrain[,c('SEGMENTNO','STRELEMAX', 'CATELEMAX' )], by.x='SegmentNo', by.y='SEGMENTNO', all.x=TRUE)
		tascele=make.asc(pos[,'STRELEMAX'])
		zlim=c(min(c(pos$STRELEMAX),na.rm=T),max(c(pos$STRELEMAX),na.rm=T))
		plot(Ozoutline , lwd=10, ann=FALSE,axes=FALSE, add=TRUE)
		image(tasc,ann=F,axes=F,col=cols,zlim=zlim, add=TRUE,xlim=xlim,ylim=ylim,xpd=FALSE)
		image(tascele,ann=F,axes=F,col=cols,zlim=zlim,add=TRUE,xlim=xlim,ylim=ylim,xpd=FALSE)
		plot(rivers , lwd=3, ann=FALSE,axes=FALSE, add=TRUE, col="#1C86EE")
		text(115,-10.635,"(c)",cex=14)
		zlim=c(0,2200)
		color.legend(114,-16.0,122.5,-15.0,zlim,cols,cex=8)
	...
	mat = matrix(c( 1,1,1,1,1,2,2,
					...
					1,1,1,1,1,3,3,
					5,5,5,1,1,4,4,
					5,5,5,5,5,4,4),nr=6,nc=7,byrow=TRUE) #create a layout matrix for images
    layout(mat) #call layout as defined above 
	
	# Turnover plot
	
	par(mar=c(5,3,0,3)) #define the plot parameters
	all.cols = colorRampPalette(c("#A50026","#D73027","#F46D43","#FDAE61","#FEE090","#FFFFBF","#E0F3F8","#ABD9E9","#74ADD1","#4575B4","#313695"))(21)
	cols = all.cols[11:1] # blue to red		
	pos=tpos
	load(paste('/home/jc246980/SDM/TurnOver/',es,"_",tax,'_Turnover_quants_gains.Rdata',sep='')) # Load turnover
	...
	image(tasc,ann=F,axes=F,col=cols,zlim=zlim,add=TRUE)
	plot(Drainageshape , lwd=10, ann=FALSE,axes=FALSE, add=TRUE)
	color.legend(115,-39,137,-38,zzlim,cols,cex=10)
	text(125,-36.5,"Turnover",cex=15)
	text(117,-10.5,"Frogs",cex=22)
	
	
	#Current
	par(mar=c(0,3,0,3)) #define the plot parameters
	cols=colorRampPalette(c("#A50026","#D73027","#F46D43","#FDAE61","#FEE090","#FFFFBF","#E0F3F8","#ABD9E9","#74ADD1","#4575B4","#313695"))(11)
	load(paste('/home/jc246980/SDM/Richness/Clip4North/',tax,'/RCP3PD_Richness_current.mat.Rdata',sep='')) # Current
	pos=tpos
	cols=colorRampPalette(c("#A50026","#D73027","#F46D43","#FDAE61","#FEE090","#FFFFBF","#E0F3F8","#ABD9E9","#74ADD1","#4575B4","#313695"))(20)
	...
	par(mar=c(20,15,15,10),mgp=c(0,5,.5))
	image.dir = '/home/jc246980/SDM/Paper_images/'; setwd(image.dir)
	...
	tdata$proportion=(tdata[,4]/tdata[,2])
	if (tax=="fish") hist(tdata$proportion, breaks=20,main="", xlim=c(0,6), ylab=NA,xlab=NA,col="grey", lwd=10, axes=F)
	if (tax=="crayfish") hist(tdata$proportion, breaks=20,main="", xlim=c(0,6), ylab=NA,xlab=NA,col="grey", lwd=10, axes=F)
	if (tax=="turtles") hist(tdata$proportion, breaks=20,main="", xlim=c(0,6), ylab=NA,xlab=NA,col="grey", lwd=10, axes=F)
	if (tax=="frog") hist(tdata$proportion, breaks=20,main="", xlim=c(0,6), ylab=NA,xlab=NA,col="grey", lwd=10, axes=F)
	mtext(side = 1, "Proportional change in suitable habitat", line = 15, cex=8)
	mtext(side = 2, "Frequency", line = 13, cex=8 )
	axis(side = 1, tck = 0.015,lwd = 10,labels=TRUE, cex.axis=7)
	axis(side = 2, tck = 0.015,lwd = 10,labels=TRUE,cex.axis=7)
	abline(v=1,col="red",lwd=10)
