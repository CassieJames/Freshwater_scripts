...
clip=clipnew
colnames(clip)=c("SegmentNo", "Clip2Bio", "Clip2RB")
...
regions2add=clipping_action[which(clipping_action$Species==spp),5] # determine ammendments as regions to add
regions2subtract=as.character(regions2subtract)
...
regions=unique(clip[which(clip$SegmentNo %in% occur$lat),clip.column])#find the regions in which species has been observed 
...
colnames(distdata)=c('SegmentNo','Current')
save(distdata,file=paste(out,"/",spp,'.cur.real.mat.trial.Rdata',sep='')); rm(distdata); gc() #write out the data		
}
