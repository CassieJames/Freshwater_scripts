...
species.name=sub(".cur.real.mat.Rdata","", species)	
...
# Determine current species richness
	...
	load(paste(real.dir, spp,'.cur.real.mat.Rdata',sep='')) #load the current realised distribution data. 
	real.mat[which(real.mat[,2]>0),2]=1 #clip anything above threshold to 1
	if(spp==species[1]) real.mat=real.mat else real.mat[,'SegmentNo'] = 0
	if(spp==species[1]) Richness_current=real.mat[,1:2] else Richness_current=Richness_current+real.mat [,1:2]

# determine future species richness

	load(paste(real.dir,spp,'.cur.real.mat.Rdata',sep='')) #load the future realised distribution data. object is called real.mat
	fut.mat=real.mat[,2:146]
