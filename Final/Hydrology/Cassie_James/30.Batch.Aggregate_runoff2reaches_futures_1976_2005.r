...
for(es in ESs) { 
	for(gcm in GCMs) {		
		...
		zz = file(paste('30.',es,'.',gcm,'.aggregate.sh',sep=''),'w')
			 ...
			 cat("R CMD BATCH --no-save --no-restore '--args es=\"",es,"\" gcm=\"",gcm,"\" data.dir=\"",data.dir,"\" out.dir=\"",out.dir,"\" ' ~/Freshwater_scripts/Final/Hydrology/Cassie_James/30.Aggregate_runoff2reaches_futures_1976_2005.r 30.",es,'.',gcm,'.Rout \n',sep='',file=zz) #run the R script in the background
		close(zz) 
