###################################################################################################
#### Script to prepare hydrology for SDM
#### C. james
	...
	for (es in ESs) {
		for (gcm in GCMs) {
			...
			for (yy in YEAR) {
				...
				runoff.data$Annual_mean =rowMeans(runoff.data[,c(2:13)]) # determine annual mean runoff
				...
				tdata$Annual_mean=rowMeans((tdata[,c(2:13)]+1)*((tdata[,c(26:37)])/(tdata[,c(14:25)]+1))) # apply correction
